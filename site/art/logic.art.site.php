...
include_once("define.art.site.php");	
...
define('ART_NAME', ART_SITE_MOD_NAME);
...
function itemListShow($tag){ //Выборка всех видимых итемов (showing)
...
function itemListShowHit($tag){ //Выборка всех видимых (showing) итемов отмеченных полем (hit) 

    $query = "SELECT * FROM ".ART_ITEM_TABLE." 
              LEFT JOIN ".ART_ITEM_TABLE_LANG." ON ".ART_ITEM_TABLE.".id=".ART_ITEM_TABLE_LANG.".pid 
              WHERE `connect` LIKE '%;".$tag.";%' AND `showing`='1' AND `hit`='1' ORDER BY `pos` DESC";
              
//    $query = "SELECT * FROM ".ART_ITEM_TABLE." WHERE `connect` LIKE '%;".$tag.";%' AND `showing`='1' AND `hit`='1' ORDER BY `pos` DESC";
...
function itemListCompShow($tag){
    ...
    $query = "SELECT * FROM ".ART_ITEM_TABLE." 
              LEFT JOIN ".ART_ITEM_TABLE_LANG." ON ".ART_ITEM_TABLE.".id=".ART_ITEM_TABLE_LANG.".pid 
              WHERE `companion_connect` LIKE '%;".$tag.";%' AND `showing`='1' ORDER BY `pos` DESC";
    $res = mysql_query($query);
    Mysql::queryError($res,$query);
  //  $row = mysql_fetch_array($res);
		if (mysql_num_rows($res) > 0)
...
//    $query = "SELECT * FROM ".ART_ITEM_TABLE." WHERE `id`='".$id."' LIMIT 1";              
