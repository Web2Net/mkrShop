...
<table width="100%" class="price">
    <tr>
        <td style="height:90px;" valign="bottom" align="center">
            <div style="margin-bottom:12px;"><?=NOTE_PRICE?>: 
<?if($price_roznica!=0 && $nalichie != "arch"){?>
                <span style="color: #000;"><?=$dp[0]?>.<font><?=$dp[1]?></font></span> <?=CURRENCY?>
<?}elseif($nalichie == "arch"){?>
                <span style="font-size:17px;line-height:25px;padding:7px;">Снят с производства</span>
<?}else{?>
