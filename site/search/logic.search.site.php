...
*/
        if(isset($_SERVER['REQUEST_URI'])){
            ...
            if(isset($seodata[0])) $mod=$seodata[0];  
            if(isset($seodata[1])) $rub=$seodata[1];  
            if(isset($seodata[2])) $item = $seodata[2];
...
//SYS::varDump($search_list_product,__FILE__,__LINE__,"search_list_page");		
        if(isset($search_list_art) == NULL && isset($search_list_item) == NULL)
        {
...
if($level == 0){
SYS::varDump($query,__FILE__,__LINE__,"query");       
}        
        $res = mysql_query($query);
	mysql::queryError($res,$query);
...
*/
        $query = "SELECT caption FROM ".SEARCH_ITEM_TABLE." 
                  WHERE `flagman`='1' AND `showing`='1'";
