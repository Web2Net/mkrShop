...
class Price
{
	const TB_SHOP_ITEM_TEMP = 'shop_item_temp';
	private $shop_item_temp = 'shop_item_temp';
...
/*
                ...
                Price::checkFile($file); // Удаляем повторяющиеся строки из файла
//Price_erc::updateImgFromFile($file);  // сохранение изо с удаленного адреса           
                Mysql::deleteAllFields(self::TB_SHOP_ITEM_TEMP); // Удаление всех записей из временной таблицы
                $change_price = Price_erc::createTempTable(self::TB_SHOP_ITEM_TEMP,$file,$postavschik);  // Записываем данные из файла во временную таблицу 
                $tpl_item->assign('change_price',$change_price);
                $mess = "Обновленна временная таблица. Поставщик ERC.";
                $tpl_item->assign('mess',$mess);
                Price::deleteSomeFilds(self::TB_SHOP_ITEM_TEMP); // Удаление из таблицы записей с некоторыми пустыми полями
*/
            }
            else{
                exit("{$file} - нету такого..");
