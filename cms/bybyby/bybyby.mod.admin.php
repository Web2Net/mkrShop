<?
define('PATH_BYBYBY_TPL','/cms/bybyby/tpl/');
define('PATH_BYBYBY_JS','/cms/bybyby/JS/');

define('TABLE_BYBYBY_MAIN','bybyby_main');

class Bybyby
{
    static function admBybyby(){
...
//SYS::varDump($_POST,__FILE__, __LINE__, "POST");
        if(isset($ext) && $ext=="ajax"){
	        if(isset($_FILES["file"]) && trim($_FILES["file"]["name"])!= ""){	
                ...
                $mess=Bybyby::loadFile($_FILES["file"]);
                if($mess=="OK")
                {
	                $file_dir = "/cms/by/doc/";
...
//SYS::varDump($row,__FILE__,__LINE__,"Bybyby");            
...
// Поиск по Клиенту	
                ...
                if($type == "client"){
                    $row=Bybyby::getList("`client` LIKE '%{$get_client}%' OR `id` LIKE '%{$get_client}%'");
                }
// /Поиск по Клиенту	              
             
                $tpl_item->assign('listing',$row);
			    $c_cont=$tpl_item->get("bybyby-list");
        
            }
            elseif($com=="add"){
	            $row=Bybyby::getBybyby($id="");
                $tpl_item->assign('art_data',$row);
	            $c_cont=$tpl_item->get("bybyby-add-edit");
	        }
            elseif($com=="edit"){
	            $row=Bybyby::getBybyby($id);
                $tpl_item->assign('art_data',$row);
...
//SYS::varDump($c_cont,__FILE__,__LINE__,"c_cont");		   
            ...
            elseif($com=="update"){
                foreach($_REQUEST as $key=>$val){
                    ...
                    if($var['0'] == "form2"){ // форма, прилетевшая из файла toner.php для табл. bybyby_in_toner
                        $arr_value_2[$var['1']] = Text::cut($val);
                    }
                    if($var['0'] == "form3"){ // форма, прилетевшая из файла drum_unit.php для табл. bybyby_in_dr_unit
                        $arr_value_3[$var['1']] = Text::cut($val);
                    }
                    if($var['0'] == "form4"){ // форма, прилетевшая из файла developer.php для табл. bybyby_in_developer
                        $arr_value_4[$var['1']] = Text::cut($val);
                    }
                    if($var['0'] == "form5"){ // форма, прилетевшая из файла developer_unit.php для табл. bybyby_in_d_unit
                        $arr_value_5[$var['1']] = Text::cut($val);
                    }
                    if($var['0'] == "form6"){ // форма, прилетевшая из файла pechka.php для табл. bybyby_in_pechka
                        $arr_value_6[$var['1']] = Text::cut($val);
                    }
                    if($var['0'] == "form7"){ // форма, прилетевшая из файла bynker.php для табл. bybyby_in_bynker
                        $arr_value_7[$var['1']] = Text::cut($val);
                    }
                    if($var['0'] == "form8"){ // форма, прилетевшая из файла lenta.php для табл. bybyby_in_lenta
                        $arr_value_8[$var['1']] = Text::cut($val);
                    }
                    
                    if($var['0'] == "form9"){ // Запись в  табл. bybyby_out (таблица продажи)
...
//SYS::varDump($arr_value,__FILE__,__LINE__);
                  ...
                  $table = "bybyby_main";
                  $table_2 = "bybyby_in_toner";
                  $table_3 = "bybyby_in_dr_unit";
                  $table_4 = "bybyby_in_developer";
                  $table_5 = "bybyby_in_d_unit";
                  $table_6 = "bybyby_in_pechka";
                  $table_7 = "bybyby_in_bynker";
                  $table_8 = "bybyby_in_lenta";

                  $table_9 = "bybyby_out";  // табл. продаж
...
//exit("Stop");
                      ...
                      $where_2['bybyby_id'] = $arr_value['id']; 
// TONER Запись в  табл. bybyby_in_toner                     
                      if(is_array($arr_value_2)){
                          $res_2 = $db->updateSQL ($table_2, $arr_value_2, $where_2);
                      }     
// /TONER Запись в  табл. bybyby_in_toner

// DRUM_UNIT Запись в  табл. bybyby_in_dr_unit
...
// DEVELOPER Запись в  табл. bybyby_in_developer                     
                      if(is_array($arr_value_4)){
                          $res_4 = $db->updateSQL ($table_4, $arr_value_4, $where_2); 
                      }    
// /DEVELOPER Запись в  табл. bybyby_in_developer

// Запись в  табл. bybyby_in_d_unit                     
...
// Запись в  табл. bybyby_in_pechka                     
...
//$arr_value['date_out'] = date("Y-m-d")." ".$_SESSION['user_login'];
                          ...
                          $id = $db->insertSQL ($arr_value, $table); // Создается новая запись в таблице bybyby
                        

                      $bybyby_id = mysql_insert_id(); // Получаем id последней записи из табл. bybyby
                      $arr_value_2['bybyby_id'] = $bybyby_id; 

                      
                          $id_2 = $db->insertSQL ($arr_value_2, $table_2); // Создаем новые записи во всех дочерних таблицах с привязкой bybyby_main.id => tabl.bybyby_id 
                          ...
                          $id_9 = $db->insertSQL ($arr_value_2, $table_9); // Создаем новую запись в табл. bybyby_out
...
//SYS::varDump($_SESSION['ingener'],__FILE__,__LINE__,"SESSION");
    ...
    static function getList($var){
        ...
        $db = new mysql;
        $row = $db->origSelectSQL($select, TABLE_BYBYBY_MAIN, $where, $sortby);
...
//exit($where);        
        ...
        $db = new mysql;
        $row = $db->origSelectSQL($select, TABLE_BYBYBY_MAIN, $where, "");
SYS::varDump($row,__FILE__,__LINE__,"getBybyby()"); 
        return $row[0];       
    }
