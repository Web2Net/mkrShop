...
class Text 
{
    ...
    static function repTN($string) // исключительно для правки прайсов от Минолта..
    {
        $string = trim($string);
        $string = ereg_replace("-","",$string);
...
// Из текста $text удаляем слова короче $wordLen символов
    static function deleteShortWords($text,$wordLen){ 
        $words = explode(" ", $text);
        foreach($words as $key => $value){
            if(strlen($value) < $wordLen) $errorWords[] = $value;
        }
        $text = str_replace($errorWords, "", $text);
        return $text;    
    }
...
//------------------------------------------------- Перевод чисел: Арабские в Римские и обратно -----------------------------//
    ...
    static function linksKill($var){
        $var = preg_replace('@((https?://)?([-\w]+\.[-\w\.]+)+\w(:\d+)?(/([-\w/_\.]*(\?\S+)?)?)*)@', '', $var);
        return $var;
    }
