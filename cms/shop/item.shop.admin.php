...
include (SITE_PATH."/cms/inc/eval.php");
        ...
        $item_list = new ShopItem; 
        $tag_list = new ShopTag;
        $text = new Text;
        $db = new mysql;
        $lang = new Lang;
        ...
        if($com == "list"){	
            ...
            if(isset($display) && $display !== ""){ // В $display сидят всяческие условия, фильтры, порядок вывода и т.д.

                $_SESSION["display"] = "";

                if($display=="full"){
                    $item_list =  $item_list->itemListIdData($tag,$filter);
                }
                elseif($display=="nalichie_y"){
                    $item_list =  $item_list->itemListSomeField("nalichie", "+", $tag, $filter);
                }
                elseif($display=="nalichie_n"){
                    $item_list =  $item_list->itemListSomeField("nalichie", "-", $tag, $filter);
                }
                elseif($display=="nalichie_r"){
                    $item_list =  $item_list->itemListSomeField("nalichie", "r", $tag, $filter);
                }
                elseif($display=="nalichie_arch"){
                    $item_list =  $item_list->itemListSomeField("nalichie", "arch", $tag, $filter);
                }
                else{
                    $item_list = $item_list->itemListIdData($tag,$filter);
                }
            ...
            if(INI::Life('shop_tag_list')>30){
                $tag_list = $tag_list->tagListIdData();
        ...
        if($cmd == "update" || $cmd == "insert"){
            ...
            $arr_value['p5']=$p5; // В магазине MKR используется как неизменная позиция при обновлении прайс-листов
            $arr_value['showing']=$showing;
            $arr_value['p1']=$p1; // В  магазине MKR используется только в связке с полем поставщик, иф $postavschik=mkr - цена в грн, элс - цена в евро
            ...
            $arr_value['meta_t'] = $text->createMetaTitle($meta_t, $caption);  // Формируем meta_title            
            $arr_value['meta_d'] = $text->createMetaDescription($meta_d, $desc_short, $desc_full, $caption);  // Формируем meta_description
            $arr_value['meta_k'] = $text->createMetaKey($arr_value2['meta_d']);  // Формируем meta_key
            
            if($postavschik == "" || $postavschik == "mkr"){ // При ручном вводе товара, в поле "Цена дилера" сразу вводиться цена магазина и выводится на сайте без перерасчета 
                $arr_value['price_roznica']=$price_diler; 
            }
	    
            if($postavschik == ""){
		        $arr_value['postavschik'] = "mkr";
	        }else{
		        $arr_value['postavschik'] = strtolower($postavschik);
	        }
...
SYS::varDump($p1,__FILE__,__LINE__,'P1');            
            
            if(!isset($package) || $package == ""){ // Используется под кол-во товара на складе
                $arr_value['package']=0;
            }
            else{
...
// 20171111 Ковырялся с DDP - нужно разобраться что ниже.. Пока закомитил
/*
            $p1=$_POST["p1"];
            if($p1!=NULL){
                $param_c=";";
...
//$arr_value['article'] = $param_list[0]['value1']."-".$_POST['article']."-".$param_list[1]['value1'];
            $arr_value['article'] = $arr_value['1Cid'] = $article;
...
//*********** /Создание языковых записей *********//	
            ...
            if($com=="add"||$com=="edit"){	
                ...
                $tag1_list = $tag_list->tagListIdData(1);
                $tpl_item->assign('tag1_list',$tag1_list);

                $tag2_list = $tag_list->tagListIdData(2);
...
//$param_item_list[$val["id"]]=ParamItem::itemListIdData($val["id"]);
                ...
                if($com == "edit"){
                    $item_data = $item_list->itemData($id);
...
function itemListIdData($tag,$filter){
    ...
    if(isset($filter) && $filter !== ""){
        ...
        $order = " ORDER BY price_diler ASC, flagman DESC, pos DESC";
	}
	else{
		$filter = "";
        $order = " ORDER BY price_diler ASC, flagman DESC, pos DESC";
    ...
    $query = "SELECT * FROM ".SHOP_ITEM_TABLE."  
              WHERE `connect` LIKE '%;" . $tag . ";%'". $filter . $order;
SYS::varDump($query,__FILE__,__LINE__,'ShopItem::itemListIdData::query');              
    $res = mysql_query($query);
    ...
    static function getList($table){
        ...
        $from = $table;
        $where="`showing` = '1'";
        $sortby="caption_full DESC";

        $db = new mysql;
        $row = $db->origSelectSQL($select, $from, $where, $sortby);
SYS::varDump($row,__FILE__,__LINE__,"SHOP::getList");
        return $row;
    } 

    static function itemListSomeField($field, $value, $tag, $filter){
        ...
        $query = "SELECT * FROM ".SHOP_ITEM_TABLE."  
                  WHERE `connect` LIKE '%;".$tag.";%' AND `".$field."` = '".$value."' {$filter} ORDER BY article ASC,flagman DESC, pos DESC";
...
static function getFilter($filter_list_id,$razdelitel){
    ...
    static function getBrand($tag_id){
    	...
    	if(isset($tag_id) && $tag_id !== "0"){
			...
			$query = "SELECT DISTINCT `brand`  FROM `shop_item` ORDER BY `brand`";
		}
