...
//       $file = Dir::isFile($file);
        ...
        {                                                        
/*
        	$stroka = explode($values,",");
			$count_stoka = count($stroka);
			if($count_stoka !== "31"){
				unset($values);
			}
			else{
*/				
			if(!isset($values[4]) || $values[4] == "" || $values[4] == "Код" || !isset($values[6]) || $values[6] == "0,00" || !isset($values[8]) || $values[8] == "0,00"){// удаление полей с пустым кодом товара (код производителя), нулевой ценой
				unset($values);
			} 
            else
			{
                if($values[0] == "Cooler Master" || 
                   $values[0] == "Silicon Power" || 
                   $values[0] == "Neo Tools" || 
                   $values[0] == "Same Toy" ||
                   $values[0] == "Sequin Art" ||
                   $values[0] == "Smart Koala" ||
                   $values[0] == "Soft toy" ||
                   $values[0] == "Light Stax" ||
                   $values[0] == "Mi Baby"){
                ...
                elseif($values[0] == "" || $values[0] == "Сувениры и подарки"){
                    $brand = "noname";
                }
				else{
...
/*
				for ($i = 0; $i <= 40; $i++) {
                    if(isset($values[$i]) && $values[$i] !== ""){$values[$i] = strip_tags($values[$i]);}
                    if(isset($values[$i]) && $values[$i] !== ""){$values[$i] = htmlspecialchars($values[$i]);}
                }
*/

$filtr = "";
$filter_connect = "";
include SITE_PATH."/cms/price/erc/erc_filter_id.php";
$category_id = "";				
include SITE_PATH."/cms/price/erc/erc_category_id.php";
...
N 13 - Наличие	
O 14 - Ожидается	
P 15 - Участвует в акции	
Q 16 - Условия акции	
R 17 - В упаковке	
S 18 - Товаров со скидкой (повр. упакаковка)	
T 19 - Описание	
U 20 - Комплектация
V 21 - Доп. инфо
W 22 - URL изображения
X 23 - Ед. изм.
Y 24 - НДС
Z 25 - EAN
AA 26 - UPC
...
if(isset($values[1]) && $values[1] !== ""){   // Категория
    $category = $values[1];
}
if(isset($values[2]) && $values[2] !== ""){   // Подкатегория
    $subcategory = $values[2];
}
if(isset($values[3]) && $values[3] !== ""){   //  Наименование 
    $caption_full = strip_tags(addslashes($values[3]));
}
if(isset($values[4]) && $values[4] !== ""){   //  Код 
    $item_1Cid = str_replace("/", "_", $values[4]);
}
if(isset($values[5]) && $values[5] !== ""){   //  ТНВЭД
     $tnwed = $values[5];
}
$img = "{$item_1Cid}.jpg";
// Цены
...
// /Цены
if(isset($values[7]) && $values[7] !== ""){   //Монитор
    $monitor = $values[7];
}
if(isset($values[9]) && $values[9] !== ""){   // DDP
    $ddp = $values[9];
}
if(isset($values[10]) && $values[10] !== ""){ //Гарантия (мес.)
    $garantiya = $values[10];
}
// Наличие
if(isset($values[12]) && $values[12] !== ""){    // Склад
    if($values[12] == "Есть"){$nalichie = "+";} 
    elseif($values[12] == "Нет"){$nalichie = "-";}
   // elseif($values[11] == "Выписано"){$nalichie = "Выписано";}
    else{$nalichie = "r";}
}
if(isset($values[13]) && $values[13] !== ""){  //Наличие (not use)
    $ostatki = $values[13];
}
// /Наличие
if(isset($values[15]) && $values[15] !== ""){ // Участвует в акции
    $note_1 = $values[15];
}
if(isset($values[16]) && $values[16] !== ""){ // Условия акции
    $note_2 = $values[16];
}
//$values[19] = preg_replace('#<a href=(?:.*)(?=</a>)#Usi', '', $values[18]);  //Описание - удаляем все ссылки 

if(isset($values[19]) && $values[19] !== ""){     //Описание
    $description_full = Text::linksKill($values[19]);
...
$meta_d = "";
//$meta_d = strip_tags(addslashes($description_full));
$meta_k = "";
/*
            if($meta_d !== 0)
            {
...
////////////////////////////////////////////////////////////////////////////////
    ...
    static function updateImgFromFile($file){  // Закачка изо с удаленного сервера (Данные в файле)
        ...
        {                                                        
            if(isset($values[16]) && $values[16] !== "" && isset($values[21]) && $values[21] !== "")
            {
                $img_url = trim($values[21]); // url изо в прайсе
                $home_dir = SITE_PATH."/userfiles/product-image";
                GD::saveImageFromRemoteUrl($img_url,$home_dir);
		    }
		    else
		    {}
