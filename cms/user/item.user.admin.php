...
// ------------ /level --------------             
            ...
            $arr_value['short_caption'] = Text::cut($short_caption);
			...
			$arr_value['organization_1'] = Text::cut($organization_1);
			$arr_value['organization_2'] = Text::cut($organization_2);
			$arr_value['organization_3'] = Text::cut($organization_3);
			$arr_value['organization_4'] = Text::cut($organization_4);
			$arr_value['edrpoy'] = $edrpoy;
			$arr_value['edrpoy_1'] = $edrpoy_1;
			$arr_value['edrpoy_2'] = $edrpoy_2;
			$arr_value['edrpoy_3'] = $edrpoy_3;
			$arr_value['edrpoy_4'] = $edrpoy_4;
...
// /Tpl change password	
    ...
    function itemListIdData($tag){
        ...
        $from=MOD_ITEM_TABLE;
        $where="connect LIKE '%;".$tag.";%'";
        $sortby="short_caption, pos DESC";

        $db = new mysql;
        $row = $db->origSelectSQL($select, $from, $where, $sortby,"0,70000");
...
//SYS::varDump($user_action,__FILE__,__LINE__,"user_action");
	...
	static function getUserData($id){  // Додумать!!!!!!!!!!!!!!!!!!!!!!!!!!
        $query = "SELECT * FROM `user_action` WHERE `id`='{$id}'";
...
//SYS::varDump($user_action,__FILE__,__LINE__,"user_action");
	...
	static function getUserChangeCurrency(){
        $query = "SELECT * FROM `user_action` WHERE `action` LIKE '%shop_setting%' ORDER BY `id` DESC";
        $res = mysql_query($query);
        mysql::queryError($res,$query);
...
////////////////////////////// 
    ...
    static function writeLog($sql){

        $file = getenv("DOCUMENT_ROOT") . "/magic/" . $_SESSION['user_login'] . "_do.log";
