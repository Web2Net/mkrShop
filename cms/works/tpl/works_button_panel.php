...
<?//if($art_data['ingener'] !== "" && $art_data['responsible_ingener']){
    foreach($_SESSION["user"]["ingeners_data"] as $ingeners_data_k => $ingeners_data_v){
	    if($art_data['ingener'] == $ingeners_data_v["login"]){
?>
		                <td class="wrap_user_panel">
                          <div class="user_panel_butt r5">
                            <img src="/image/user/item/<?=$ingeners_data_v["id"]?>/1s.png" style="width:60px" />
                        </td>
<?
	    }
    }
...
<!--	      				<td class="wrap_user_panel">
                          <div class="user_panel_butt r5">
                            <img src="/image/user/item/<?=$ingener_id?>/1s.png" style="width:60px" /><?=$ingener_name?>
                        </td> -->
<?//}?>
...
<?if($art_data['prioritet'] == "N"){?>
                            <a href="?mod=<?=$mod?>&com=prioritet&resultat=y&id=<?=$art_data['id']?>"<!-- onclick="if (confirm('Вы уверенны, что сделали работу?')) location=this.href; return false;-->">
                                <img src="<?=PATH_IMG_ADMIN?>/prioritet_n.png" style="width:30px" title="Срочно в роботу.." />
...
?>   
			    <a href="?mod=<?=$mod?>&com=pay&resultat=<?=$pay_res?>&id=<?=$art_data['id']?>" <?=$pay_mes?>>
			    	...
			    	<div style="font-size:10px;"><?=$pay_author?><br><?=$pay_date?></div>
				</a>
<?
}
